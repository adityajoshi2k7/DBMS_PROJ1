...
//				view.forward(request, response);
			...
			if(request.getParameter("hw").equals("new")) {
				...
				HashMap<ArrayList,ArrayList<String>>questions=new HashMap<>();
				if(hwID == null) {
					response.sendRedirect(request.getContextPath()+"/jsp/error.jsp");
				}
				else {
					ArrayList<Integer> qSetList = new ArrayList<Integer>();
					for(String hwid : hwID) {
						qSetList = dao.fetchQSets(hwid);
						
						for(Integer qSetId:qSetList){
							ArrayList<String> qBanks=dao.fetchQuestions(qSetId,hwid);
							ArrayList temp = new ArrayList();
							temp.add(hwid);
							temp.add(qSetId);
							questions.put(temp,qBanks);
...
//						}
					...
					RequestDispatcher view = request.getRequestDispatcher("/jsp/existingHW.jsp");
					view.forward(request, response);
			...
			newHWParams[10] = request.getParameter("policy");
		...
		else if(action.equals("newHWExisting")) {
			String hidden=request.getParameter("hidden");
			   String hw_qset[]=hidden.split("@");
			   ArrayList<String> ques=dao.getAllQuestions(hw_qset[0],Integer.parseInt(hw_qset[1]));
			   String newHWParams[] = new String[50];
			    newHWParams[0] = request.getParameter("hwID");
				newHWParams[1] = request.getParameter("hwName");
				newHWParams[2] = request.getParameter("courseID");
				newHWParams[3] = request.getParameter("noOfRetries");
				newHWParams[4] = request.getParameter("startDate");
				newHWParams[5] = request.getParameter("endDate");
				newHWParams[6] = request.getParameter("pointsPerQuestion");
				newHWParams[7] = request.getParameter("penaltyPoints");
				newHWParams[8] = request.getParameter("hwType");
				newHWParams[9] = request.getParameter("policy");
			   
			   
			   dao.insertHWTable(newHWParams[0],newHWParams[1],newHWParams[4],newHWParams[5],newHWParams[3],newHWParams[7],newHWParams[6],ques.size(),newHWParams[8],newHWParams[9]);
       		   dao.insertCourseHWTable(newHWParams[0],newHWParams[2]);
       		   dao.insertQsetTable(newHWParams[0],1);
       		   for(String qid : ques) {
				    dao.insertHWQsetQbank(newHWParams[0],1,qid);
			    }
       		  response.sendRedirect(request.getContextPath()+"/jsp/homepage.jsp");
		...
		else if(action.equals("existingHW")) {
			String hw_qset = request.getParameter("qset");
			request.setAttribute("hw_qset", hw_qset);
			
			RequestDispatcher view = request.getRequestDispatcher("/jsp/newExistingHW.jsp");
			view.forward(request, response);
		}
            ...
            if(!dao.existsInHWTable(newHWParams[0])) {
            		dao.insertHWTable(newHWParams[0],newHWParams[1],newHWParams[5],newHWParams[6],newHWParams[4],newHWParams[8],newHWParams[7],q_id.length,newHWParams[9],newHWParams[10]);
            		dao.insertCourseHWTable(newHWParams[0],newHWParams[2]);
            }
            int maxQsetNo = 0;
			...
			ArrayList<ResultSet> rs=dao.viewStudentReport(request.getParameter("u_id"),request.getParameter("c_id"));
			request.setAttribute("rs", rs);
