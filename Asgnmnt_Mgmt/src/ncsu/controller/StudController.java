...
public class StudController extends HttpServlet {
	...
	private void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equals("submitExam")) {
			...
			System.out.println("KARTHIK HWID : "+hwID);
			int totPoints = 0;
			...
			if (request.getParameter("from") == null) {// for standard type hw
				...
				attemptNo = dao.getAttemptNoAdaptive(userID, hwID);
				dao.generateReport(userID, hwID, request.getParameter("qsetID"), qids.get(0), markedAnsID.get(0),
						(int) Math.ceil(((double)attemptNo+1) / 3), isCorr.get(0), (Integer) ppq.get(0), totPoints);
				...
				Integer count = Integer.parseInt(request.getParameter("count"));
					...
					if (isCorr.get(0) == 1) {
						...
						diffLevel = Integer.parseInt(ansDetails[2]) - 1;
					ArrayList<HashMap> questions = fetchquestion(hwID, dao, diffLevel);
					...
					for (Map.Entry mp : standard.entrySet()) {
						System.out.println("KARTHIK : "+diffLevel.toString());
						if (((ArrayList) mp.getValue()).get(3).equals(diffLevel.toString())) {
							request.setAttribute("type", "std");
					...
					System.out.println("KARTHIK qmap: " + qmap);
					request.setAttribute("qmap", qmap);
					request.setAttribute("qCount", count.toString());
					request.setAttribute("user_id", userID);
					request.setAttribute("hw_id", hwID);
					request.setAttribute("qset_id", request.getParameter("qsetID"));
					request.setAttribute("diff", diffLevel.toString());
					if (count < 3) {
	...
	ArrayList fetchquestion(String hw_ID, StudDao dao, Integer diffLevel) {
		Boolean recordFound = false;
		ArrayList<ResultSet> results = dao.fetchHW(hw_ID);
		...
		try {
			...
			while (std.next()) {
				...
				if (Integer.parseInt(std.getString("difficulty_level")) == diffLevel) {
			...
			while (param.next()) {
				...
				// request.setAttribute("qset_id", param.getString("QSET_ID"));
				// request.setAttribute("hw_id", param.getString("HW_ID"));
				if (Integer.parseInt(param.getString("difficulty_level")) == diffLevel) {
