...
public class StudDao {
	...
	public ResultSet fetchAllHW(String userID, String c_id) {
		...
		String sql = "select h.*,h.no_of_retries-(select NVL(MAX(r2.attempt_no),0) from responses r2 where r2.user_id=? and r2.hw_id=h.hw_id ) as \"Attempts left\" from ENROLLED_IN e,COURSES_HOMEWORKS hc,HOMEWORKS h "
	...
	public ResultSet viewDetailedReport(String u_id, String h_id, String a_no) {
		...
		String sql = "select r.q_id,(select s.q_text from STD_QUESTIONS s where r.q_id=s.q_id union select pq.q_text from PARAM_QUESTIONS pq where r.q_id=pq.q_id) as Q_TEXT ,r.attempt_no,r.is_correct, a.answer_text as marked_answer, "
				...
				+ "from RESPONSES r " + "inner join ANSWERS a "
				+ "on r.ans_id=a.ans_id and r.q_id=a.q_id and r.user_id=? and r.hw_id=? " + "inner join qbank qb "
	...
	public int getAttemptNoAdaptive(String userID, String hwID) {
		// TODO Auto-generated method stub
		
		String sql = "select count(*) from RESPONSES where USER_ID=? and HW_ID=?";
