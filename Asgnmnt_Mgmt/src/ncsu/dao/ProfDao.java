...
public class ProfDao {
	...
	static PreparedStatement ps = null, ps3 = null;
	...
	public void insertHWTable(String hwID, String hwName, String startDate, String endDate, String noOfRetries,
			String penaltyPerQues, String pointsPerQues, int noOfQues, String hwType, String hw_policy) {
		// TODO Auto-generated method stub
	...
	public ArrayList<Integer> fetchQSets(String hwID) {
		// TODO Auto-generated method stub
		conn = DBUtil.getConnection();
		String sql = "select QSET_ID from QSET where HW_ID=?";
		ArrayList<Integer> qsetID = new ArrayList<Integer>();
	...
	public ArrayList<String> fetchQuestions(Integer qSetId, String hwID) {
	...
	public ArrayList viewStudentReport(String u_id, String c_id) {
		conn = DBUtil.getConnection();
		// String sql ="select r.hw_id,
		// h.hw_name,r.attempt_no,sum(r.POINTS_SCORED_PER_QUESTION) as
		// \"Marks_Obtained\" from RESPONSES r ,ENROLLED_IN e, COURSES_HOMEWORKS ch,
		// HOMEWORKS h where e.user_id=r.user_id and r.hw_id=h.hw_id and
		// e.course_id=ch.course_id and h.hw_id=ch.hw_id and e.user_id=? and
		// e.course_id=? group by r.hw_id,h.hw_name,r.attempt_no order by hw_id asc";
		String sql = new String();
		String score_policy = new String();
		String sql1 = "select distinct(hw_id) from RESPONSES where USER_ID=?";
		ResultSet rs1 = null, rs2 = null;
		ArrayList<ResultSet> al = new ArrayList();
		...
		if (conn != null) {
			try {
				...
				while (rs1.next()) {
					String sql2 = "select SCORING_POLICY from HOMEWORKS where HW_ID=?";

					ps = conn.prepareStatement(sql2);

					ps.setString(1, rs1.getString("HW_ID"));

					rs2 = ps.executeQuery();
					if (rs2.next()) {
						score_policy = rs2.getString("SCORING_POLICY");
					}
					if (score_policy.equals("best")) {
						sql = "select hw_name,hw_id ,MAX(MARKS_OBTAINED) as \"MARKS_OBTAINED\" ,scoring_policy from(select r.hw_id, h.hw_name,h.scoring_policy,r.attempt_no,sum(r.POINTS_SCORED_PER_QUESTION) as \"MARKS_OBTAINED\" from RESPONSES r ,ENROLLED_IN e, COURSES_HOMEWORKS ch, HOMEWORKS h where h.hw_id=? and e.user_id=r.user_id and r.hw_id=h.hw_id and e.course_id=ch.course_id and h.hw_id=ch.hw_id and e.user_id=? and e.course_id=? group by r.hw_id,h.hw_name,r.attempt_no,h.scoring_policy order by hw_id asc)group by HW_id,HW_name,scoring_policy";
						ps3 = conn.prepareStatement(sql);
						ps3.setString(1, rs1.getString("HW_ID"));
						ps3.setString(2, u_id);
						ps3.setString(3, c_id);
					} else if (score_policy.equals("average")) {
						sql = "select hw_name,hw_id ,AVG(MARKS_OBTAINED) as \"MARKS_OBTAINED\",scoring_policy from(select r.hw_id, h.hw_name,h.scoring_policy,r.attempt_no,sum(r.POINTS_SCORED_PER_QUESTION) as \"MARKS_OBTAINED\" from RESPONSES r ,ENROLLED_IN e, COURSES_HOMEWORKS ch, HOMEWORKS h where h.hw_id=? and e.user_id=r.user_id and r.hw_id=h.hw_id and e.course_id=ch.course_id and h.hw_id=ch.hw_id and e.user_id=? and e.course_id=? group by r.hw_id,h.hw_name,r.attempt_no,h.scoring_policy order by hw_id asc)group by HW_id,HW_name,scoring_policy";
						ps3 = conn.prepareStatement(sql);
						ps3.setString(1, rs1.getString("HW_ID"));
						ps3.setString(2, u_id);
						ps3.setString(3, c_id);
					} else {
						sql = "select h.hw_id,h.hw_name,h.scoring_policy,(SELECT SUM(R1.POINTS_SCORED_PER_QUESTION) "
							  + "FROM RESPONSES r1 " 
							  + "WHERE r1.ATTEMPT_NO=(SELECT MAX(r2.ATTEMPT_NO) FROM RESPONSES r2 where r2.hw_id=? AND r2.USER_ID=?) AND r1.USER_ID=? and r1.HW_ID=?) AS MARKS_OBTAINED " 
							  + "from responses r, ENROLLED_IN e, COURSES_HOMEWORKS ch, HOMEWORKS h " 
							  + "where e.user_id=r.user_id and r.hw_id=h.hw_id and e.course_id=ch.course_id and e.course_id=? and h.hw_id=ch.hw_id and e.user_id=? and r.hw_id=? " 
							  + "group by h.hw_id,h.hw_name,h.scoring_policy";
						ps3 = conn.prepareStatement(sql);
						ps3.setString(1, rs1.getString("HW_ID"));
						ps3.setString(2, u_id);
						ps3.setString(3, u_id);
						ps3.setString(4, rs1.getString("HW_ID"));
						ps3.setString(5, c_id);
						ps3.setString(6, u_id);
						ps3.setString(7, rs1.getString("HW_ID"));
					}
	...
	public ArrayList<String> getAllQuestions(String hw_id, int qset_id) {
		String sql = "select Q_ID from HW_QSET_QBANK where HW_ID=? and QSET_ID=?";
